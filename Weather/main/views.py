...
def get_weather(city_name, request_type, days = None):  #request_type = 'current' or 'forecast'
    ...
    if response.status_code == 200:
        ...
        if request_type == 'forecast' and 'forecast' in forecast_data:
            for day in forecast_data['forecast']['forecastday']:
                ...
                for hour in day['hour']:
                    ...
                    if time_str in time_slots.keys():
        ...
        print("Error weather data:", response.status_code)
        return None
...
def save_weekly_data(*args):
    ...
    for city_name in cities:
        ...
        if not city_object.weekly_forecast or (timezone.now() - city_object.week_updated_at) > timedelta(minutes=30):
            ...
            if forecast_data:
                ...
                if today in forecast_data:
                    ...
                    for time_slot in time_slots.keys():
                        if time_slot in hourly_data:
                            upcoming_forecast[today]['hourly_forecast'][time_slot] = {
                                'temp_c': hourly_data[time_slot]['temp_c'],
                                'condition': hourly_data[time_slot]['condition'],
                                'humidity': hourly_data[time_slot]['humidity'],
                                'rain_chance': hourly_data[time_slot]['rain_chance'],
                            }
                ...
                print(f"Weekly weather data for {city_name} has been successfully saved.")
            else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if get_weather(city_name, "current"):
                ...
                form.add_error(None, "We do not have weather data available for this city.")
    else:
        form = CitySearchForm()
